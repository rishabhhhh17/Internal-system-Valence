...
values
  ('Nimbus Health',          'M&A',   'Diligence',   'Signed',       'Sell-side','Healthcare',     180, 50000, 1.75, current_date + 75,  'Neha Jain',    'https://example.com/nimbus.pdf',  'Founders open to strategic exit; EBITDA ~12M. 3 strategics engaged.',                         now() - interval '42 days'),
  ...
  ('Quantum Edge',           'ECM',   'Marketing',   'Signed',       'Sell-side','Fintech',        250, 75000, 2.50, current_date + 60,  'Arjun Mehta',  'https://example.com/qedge.pdf',   'Pre-IPO roadshow kicking off Q2. Anchor investor list being built. 12 one-on-ones scheduled.', now() - interval '60 days'),
  ...
  ('Kavya Foods',            'M&A',   'Pitch',       'Not Required', 'Sell-side','Consumer',        80, null,  2.00, current_date + 150, 'Priya Sharma', null,                              'Family business — first contact made. Promoter open to partial exit.',                        now() - interval '7 days'),
  ('Meridian EdTech',        'PE/VC', 'Negotiation', 'Signed',       'Sell-side','EdTech',          35, 20000, 3.50, current_date + 90,  'Ananya Roy',   null,                              'Series C. 5 funds in diligence; shortlist of 2. LOIs expected next week.',                     now() - interval '55 days'),
  ('Polaris Energy',         'DCM',   'Preparation', 'Signed',       'Sell-side','Energy',         200, 60000, 1.25, current_date + 50,  'Karan Singh',  null,                              'USD notes issuance. Investor docs in final review. Moody''s rating meeting scheduled.',        now() - interval '28 days'),
  ...
  ('Orion Realty',           'PE/VC', 'Closing',     'Signed',       'Buy-side', 'Real Estate',    320, 80000, 1.50, current_date + 30,  'Vikram Patel', null,                              'Term sheet agreed. Final SPA negotiations. Expected close end of month.',                      now() - interval '110 days'),
  ...
  ('Aster Logistics',        'M&A',   'Diligence',   'Signed',       'Sell-side','Logistics',      110, 40000, 2.00, current_date + 85,  'Rohan Gupta',  null,                              'Pan-India last-mile network. 3 strategic acquirers in deep diligence.',                        now() - interval '35 days'),
  ('Solstice Financial',     'ECM',   'Preparation', 'Signed',       'Sell-side','BFSI',           450, 120000,2.25, current_date + 70,  'Rishi Kapoor', null,                              'Rights issue for capital adequacy. DRHP under SEBI review.',                                   now() - interval '22 days'),
  ('Crescent Pharma',        'M&A',   'Marketing',   'Signed',       'Sell-side','Healthcare',      75, 30000, 2.50, current_date + 95,  'Neha Jain',    null,                              'Speciality generics. Teaser out to 14 strategic + 6 financial. 9 NDAs signed so far.',         now() - interval '38 days'),
  ...
  ('Indus Semiconductors',   'PE/VC', 'Negotiation', 'Signed',       'Buy-side', 'Technology',     220, 55000, 2.00, current_date + 40,  'Vikram Patel', null,                              'Mid-cap fabless designer. LOI at $220M EV. Confirmatory diligence in flight.',                 now() - interval '65 days'),
  ...
  ('Saffron Retail',         'ECM',   'Marketing',   'Signed',       'Sell-side','Consumer',       165, 60000, 2.25, current_date + 55,  'Priya Sharma', null,                              'D2C lifestyle brand IPO. Anchor book 60% covered.',                                            now() - interval '40 days'),
  ('Lattice Cloud',          'PE/VC', 'Diligence',   'Signed',       'Sell-side','Technology',      80, 30000, 3.00, current_date + 100, 'Ananya Roy',   null,                              'SaaS DevOps. 3 PE firms in VDR. Q&A round 2 this week.',                                       now() - interval '30 days'),
  ('Meridian Hospitality',   'M&A',   'Pitch',       'Not Required', 'Sell-side','Hospitality',     55, null,  2.50, current_date + 160, 'Neha Jain',    null,                              'Boutique hotel chain. Second pitch with family office next Monday.',                           now() - interval '5 days'),
  ('Mirage Renewables',      'DCM',   'Closing',     'Signed',       'Sell-side','Energy',         300, 95000, 1.00, current_date + 20,  'Rishi Kapoor', null,                              'Green bond issuance. Regulatory clearance done. Final syndication.',                           now() - interval '88 days'),
  ('Sentinel Cyber',         'PE/VC', 'Marketing',   'Signed',       'Sell-side','Cybersecurity',   45, 20000, 3.50, current_date + 80,  'Arjun Mehta',  null,                              'Series B round. Teaser shared with 18 funds. Management roadshow starts next week.',           now() - interval '20 days'),
  ('Coastline Shipping',     'DCM',   'Preparation', 'Signed',       'Sell-side','Logistics',      180, 45000, 1.00, current_date + 65,  'Karan Singh',  null,                              'Ship-finance backed bonds. Underwriter commitment letters in draft.',                          now() - interval '25 days'),
  ...
  ('Pinnacle Realty Trust',  'DCM',   'Diligence',   'Signed',       'Sell-side','Real Estate',    240, 70000, 1.20, current_date + 60,  'Vikram Patel', null,                              'REIT follow-on debt round. Portfolio coverage due diligence by S&P.',                          now() - interval '33 days'),
  ...
  ('Celeste Jewellers',      'M&A',   'Pitch',       'Not Required', 'Sell-side','Consumer',        42, null,  2.50, current_date + 120, 'Priya Sharma', null,                              'Regional jeweller exploring PE investment. Pitch book in final draft.',                        now() - interval '3 days'),
  ('Radiant Media',          'PE/VC', 'Negotiation', 'Signed',       'Sell-side','Media',           68, 25000, 3.00, current_date + 45,  'Ananya Roy',   null,                              'Digital media rollup candidate. LOI received at $68M EV. Negotiating earn-out structure.',     now() - interval '48 days');
...
insert into public.funds (name, fund_type, hq_city, hq_country, aum_usd_m, check_size_min_usd_m, check_size_max_usd_m, sectors, stages, geographies, warmth, last_touched_at) values
  ('Peak XV Partners',              'VC',                  'Bengaluru',     'India',     9000,    5,    100,  array['Fintech','Consumer','Healthcare','SaaS'], array['Mandate','Marketing','Diligence'], array['India','SE Asia'], 'hot',     current_date - 8),
  ...
  ('Elevation Capital',             'Growth',              'Gurugram',      'India',     2000,    5,    50,   array['Fintech','Consumer','SaaS'],              array['Marketing','Diligence'],           array['India'],           'warm',    current_date - 18),
  ...
  ('Sequoia Capital',               'VC',                  'Menlo Park',    'USA',       85000,   10,   250,  array['SaaS','Fintech','Consumer Tech','AI'],    array['Marketing','Diligence'],           array['Global'],          'cold',    current_date - 150),
  ('Andreessen Horowitz (a16z)',    'VC',                  'Menlo Park',    'USA',       35000,   10,   300,  array['Fintech','SaaS','AI','Consumer Tech'],    array['Marketing','Diligence'],           array['Global'],          'cold',    current_date - 180),
  ('Tiger Global',                  'Growth',              'New York',      'USA',       60000,   25,   500,  array['Fintech','SaaS','Consumer Tech'],         array['Marketing','Diligence','Negotiation'], array['Global'],      'warm',    current_date - 28),
  ...
  ('General Atlantic',              'Growth',              'New York',      'USA',       76000,   50,   600,  array['Fintech','Healthcare','Consumer','SaaS'], array['Marketing','Diligence'],           array['Global'],          'warm',    current_date - 60),
  ('Insight Partners',              'Growth',              'New York',      'USA',       80000,   30,   500,  array['SaaS','Fintech'],                          array['Marketing','Diligence'],           array['Global'],          'cold',    current_date - 140),
  ('Coatue Management',             'Hedge Fund',          'New York',      'USA',       50000,   25,   400,  array['Fintech','Consumer Tech','SaaS'],         array['Marketing','Diligence'],           array['Global'],          'cold',    current_date - 165),
  ('KKR',                           'PE',                  'New York',      'USA',       540000,  100,  2000, array['Infrastructure','Healthcare','Consumer','Real Estate','Energy'], array['Diligence','Negotiation'], array['Global'], 'warm', current_date - 20),
  ('Blackstone',                    'PE',                  'New York',      'USA',       1000000, 100,  3000, array['Real Estate','Infrastructure','Healthcare','Consumer'],         array['Diligence','Negotiation'], array['Global'], 'cold', current_date - 105),
  ('Carlyle Group',                 'PE',                  'Washington',    'USA',       425000,  75,   1500, array['Healthcare','Consumer','Industrials'],    array['Diligence','Negotiation'],         array['Global'],          'cold',    current_date - 170),
  ('Brookfield Asset Management',   'PE',                  'Toronto',       'Canada',    925000,  100,  2500, array['Infrastructure','Real Estate','Renewables'], array['Diligence','Negotiation'],      array['Global'],          'warm',    current_date - 15),
  ('TPG Capital',                   'PE',                  'San Francisco', 'USA',       224000,  75,   1000, array['Healthcare','Consumer','Tech'],           array['Diligence','Negotiation'],         array['Global'],          'cold',    current_date - 95),
  ('Bain Capital',                  'PE',                  'Boston',        'USA',       185000,  50,   1500, array['Healthcare','Consumer','Industrials','Tech'], array['Diligence','Negotiation'],     array['Global'],          'cold',    current_date - 190),
  ('GIC',                           'Sovereign',           'Singapore',     'Singapore', 770000,  100,  2000, array['Infrastructure','Real Estate','Renewables','Healthcare'], array['Diligence','Negotiation'], array['India','SE Asia','Global'], 'warm', current_date - 30),
  ('Temasek',                       'Sovereign',           'Singapore',     'Singapore', 380000,  50,   1500, array['Healthcare','Tech','Fintech','Consumer'], array['Marketing','Diligence'],           array['India','Global'],  'warm',    current_date - 45),
  ('ADIA',                          'Sovereign',           'Abu Dhabi',     'UAE',       780000,  100,  3000, array['Infrastructure','Real Estate','Energy'],  array['Diligence','Negotiation'],         array['Global'],          'cold',    current_date - 125),
  ('Premji Invest',                 'Family Office',       'Bengaluru',     'India',     12000,   10,   200,  array['Healthcare','Consumer','SaaS','Fintech'], array['Marketing','Diligence'],           array['India','US'],      'hot',     current_date - 12),
  ...
  ('Reliance Family Office',        'Family Office',       'Mumbai',        'India',     5000,    25,   500,  array['Energy','Telecom','Consumer','Infrastructure'], array['Diligence','Negotiation'], array['India','Global'],   'cold',    current_date - 160),
  ('Tata Capital — Corp Dev',       'Strategic Corp Dev',  'Mumbai',        'India',     null,    25,   500,  array['Consumer','Industrials','Tech'],          array['Diligence','Negotiation'],         array['India','UK'],      'warm',    current_date - 30),
  ('Reliance Industries — M&A',     'Strategic Corp Dev',  'Mumbai',        'India',     null,    50,   1000, array['Telecom','Energy','Consumer','Tech'],     array['Diligence','Negotiation'],         array['India'],           'cold',    current_date - 200);
