          ...
          return (
            <div
              ...
              className={`shrink-0 w-[260px] rounded-xl border transition ${
                isOver ? 'border-valence-blue/50 bg-valence-blue-soft/40' : 'border-valence-border bg-white/[0.02]'
              }`}
            >
              <div className="flex items-center justify-between px-3 py-2.5 border-b border-valence-border">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <p className="text-[11px] font-semibold uppercase tracking-wider text-white truncate" title={stage.desc}>{stage.id}</p>
                </div>
                <span className="rounded-md border border-valence-border bg-white/[0.04] px-1.5 py-0.5 text-[10px] font-semibold text-valence-muted">
                  {items.length}
                </span>
              </div>
              ...
              <div className="p-2 space-y-2 min-h-[60px]" title={stage.desc}>
                {items.length === 0 ? (
                  ...
                  <article
                    ...
                    onClick={() => onOpen?.(d)}
                    className="group cursor-pointer rounded-lg border border-valence-border bg-valence-surface/80 p-3 transition hover:border-valence-border-strong hover:bg-white/[0.05] active:opacity-60"
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <p className="truncate text-[13px] font-semibold text-white">{d.client_name}</p>
                    </div>
                    <div className="mt-2 flex flex-wrap items-center gap-1.5 text-[10px]">
                      <span className="inline-flex items-center rounded-md border border-valence-border bg-white/[0.03] px-1.5 py-0.5 font-semibold text-valence-muted">
                        {d.deal_type}
                      </span>
                      {d.side && (
                        <span className="inline-flex items-center rounded-md border border-valence-border bg-white/[0.03] px-1.5 py-0.5 font-semibold text-valence-muted">
                      ...
                      {d.sector && (
                        <span className="inline-flex items-center rounded-md border border-valence-border bg-white/[0.03] px-1.5 py-0.5 font-semibold text-valence-muted truncate max-w-[110px]">
                          {d.sector}
                        </span>
                      )}
                    </div>
...
function Legend() {
  return (
    <div className="flex items-start gap-3 rounded-lg border border-valence-border bg-white/[0.02] px-4 py-3">
      ...
      <div className="flex-1">
        <p className="text-sm font-semibold text-white">The Valence funnel</p>
        <p className="mt-0.5 text-[11px] leading-relaxed text-valence-muted">
          Preliminary conversations sit in <b className="text-white">Origination</b> and <b className="text-white">Pitch</b>. Once we're engaged (<b className="text-white">Mandate</b>) we build materials (<b className="text-white">Preparation</b>), market the deal (<b className="text-white">Marketing</b>), run <b className="text-white">Diligence</b> and <b className="text-white">Negotiation</b>, and ship at <b className="text-white">Closing</b>. Hover any column for the full definition, or drag a card to move a deal.
        </p>
      </div>
    </div>
  )
