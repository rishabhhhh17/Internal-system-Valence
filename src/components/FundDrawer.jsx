...
export default function FundDrawer({ open, onClose, existing, onSubmit, onRename }) {
  ...
  useEffect(() => {
    ...
    // Clear the previous fund's data BEFORE fetching so switching funds doesn't
    // flash fund A's contacts/pings/people under fund B's header.
    setContacts([]); setPings([]); setPeopleAtFund([])
  ...
  return (
    ...
    >
      {existing && (
        <div className="mb-5 -mx-1 flex items-center gap-1 overflow-x-auto border-b border-valence-border">
          {TABS.map(t => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className={`relative shrink-0 whitespace-nowrap px-3 py-2 text-xs font-semibold transition ${tab === t.id ? 'text-valence-text' : 'text-valence-muted hover:text-valence-text'}`}
            >
              {t.label}
...
function ContactsTab({ fundId, contacts, setContacts }) {
  ...
  async function add(e) {
    ...
    setBusy(false)
    // Don't fail silently — a dead "Add" button reads as broken. Surface why.
    if (error) { toast.error(humanError(error, 'Could not add contact')); return }
