...
export default function People() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Page hero — consistent with every other page (eyebrow + display
          H1 + a one-line stat ribbon), so navigating between sections
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1.5 text-sm text-valence-muted tabular-nums">
            {rows.length} {rows.length === 1 ? 'person' : 'people'} tracked
          </p>
        </div>
        ...
        <tbody>
          {rows.map(p => {
            ...
            <tr key={p.id} className="border-t border-valence-border/60 hover:bg-valence-surface/60">
              ...
              <td className="px-3 py-3 text-valence-muted"><div className="truncate max-w-[220px]">{[p.role, p.company].filter(Boolean).join(' · ') || '—'}</div></td>
              <td className="px-3 py-3">{scoreMap?.get(p.id) ? <WarmthChip score={scoreMap.get(p.id)} showScore /> : '—'}</td>
              <td className="px-3 py-3 text-valence-muted"><div className="truncate max-w-[160px]">{locationLine(p) || '—'}</div></td>
              <td className="px-3 py-3 text-valence-muted"><div className="truncate max-w-[200px]">{(p.tags || []).join(' · ') || '—'}</div></td>
              <td className="px-3 py-3 text-valence-muted"><div className="truncate max-w-[220px]">{p.email || '—'}</div></td>
