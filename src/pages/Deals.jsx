  ...
  return (
    <div className="space-y-5">
      ...
      <div className="relative">
        <div className="flex items-center gap-1 overflow-x-auto rounded-lg border border-valence-border bg-valence-surface p-1">
          {primaryTabs.map(t => (
            <button
              key={t.id}
...
// Readable deal "Type" from the CURRENT model (deal_subtype + deal_types[]),
// falling back to the legacy singular `deal_type` for old rows. Mirrors the
// Analytics label logic so the export matches what's shown on-screen.
function csvType(d) {
...
// The headline size depends on the deal's subtype: a fundraise reports the
// raise, M&A the target valuation, an exit the exit target. Fall back through
// the legacy ticket field so old rows still export a number.
function csvSize(d) {
  const v = d.deal_subtype === 'fundraise' ? d.target_raise_usd_m
    : d.deal_subtype === 'exit'            ? d.target_exit_usd_m
    : d.deal_subtype === 'm_and_a'         ? d.target_valuation_usd_m
    : (d.target_raise_usd_m ?? d.target_valuation_usd_m ?? d.ticket_size_usd_m)
  return v ?? ''
}

function exportCSV(deals) {
  if (!deals?.length) return
  const header = ['Client','Type','Side','Stage','Sector','Geography','Size (USDm)','Valuation (USDm)','NDA','Lead','Target close','Website','Notes']
