    ...
    <div className="relative border-r border-valence-border last:border-r-0">
      ...
      <div className="relative" style={{ height: totalHeight }}>
        ...
        {laidOut.filter(ev => ev._lane < MAX_VISIBLE_LANES).map(ev => {
          ...
          const startMin = Number.isNaN(start.getTime())
            ? 0
            : (start.getHours() + start.getMinutes() / 60 - DAY_START_HOUR) * HOUR_HEIGHT
          const rawDuration = differenceInMinutes(end, start)
          const durationMins = Number.isFinite(rawDuration) && rawDuration > 0 ? rawDuration : 30
