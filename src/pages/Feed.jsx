...
export default function Feed() {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Only show the inline banner when we HAVE items but a refresh failed —
          the empty+error case is handled by its own retry state below, so the
          two never stack contradictorily ("failed" over "No activity yet"). */}
      ...
      {loading && items.length === 0 ? (
        ...
        <div className="vl-card p-8 text-center">
          <ActivityIcon className="mx-auto h-6 w-6 text-valence-danger" />
          <p className="mt-3 text-sm font-semibold text-valence-text">Couldn't load the activity feed.</p>
          <p className="mt-1 text-xs text-valence-muted">{error}</p>
          <button onClick={load} className="vl-btn-secondary mx-auto mt-5"><RefreshCw className="h-3.5 w-3.5" /> Try again</button>
        </div>
      ) : filtered.length === 0 ? (
