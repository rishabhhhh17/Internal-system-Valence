...
export default function NotificationsPage() {
  ...
  const fetchPage = useCallback(async (pageIndex, append) => {
    ...
    try {
      ...
      // Don't let a failed query render as "Nothing here" — surface a retry.
      ...
      {rows.length === 0 && isLoading ? (
        <ul className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <li key={i} className="h-[68px] rounded-xl border border-valence-border bg-valence-elevated animate-pulse" />
          ))}
        </ul>
      ) : rows.length === 0 && error ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div className="mb-4 grid h-12 w-12 place-items-center rounded-full bg-valence-danger/10 ring-1 ring-valence-danger/30">
            <BellOff className="h-5 w-5 text-valence-danger" />
          </div>
          <h3 className="text-base font-semibold text-valence-text">Couldn't load notifications</h3>
          <p className="mt-1 max-w-sm text-sm text-valence-muted">{humanError(error, 'Something went wrong.')}</p>
          <button onClick={() => { setError(null); fetchPage(0, false) }} className="vl-btn-secondary mt-5">
