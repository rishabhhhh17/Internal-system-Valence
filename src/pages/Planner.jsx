...
export default function Planner() {
  ...
  const [gTasksError, setGTasksError] = useState(false)  // Google Tasks scope missing/expired
  ...
  async function loadGoogleTasks() {
    if (!googleConnected) { setGTasks([]); setGTasksError(false); return }
    try {
      const items = await listGoogleTasks()
      setGTasks(items)
      setGTasksError(false)
    } catch (e) {
      // Task scope might be missing/expired. Don't fail silently — the user's
      // Google tasks would vanish from the merged list with no explanation.
      // Flag it so the Tasks panel header can prompt a reconnect.
            ...
            {loading ? (
              ...
              <div className="mt-4">
                <EmptyInline
                  ...
                  body="No upcoming meetings scheduled yet. Propose a time to a counterparty and it'll appear here."
                />
              </div>
            ) : (
        ...
        <section className="vl-card p-6 h-fit">
          <div className="flex items-center justify-between">
            <div>
              ...
              {gTasksError && (
                <p className="mt-1 inline-flex items-center gap-1 text-[11px] text-valence-warning">
                  <AlertTriangle className="h-3 w-3" /> Reconnect Google to sync your Google Tasks.
                </p>
              )}
            </div>
          </div>
            ...
            <div className="mt-5 space-y-4">
              ...
              {openTaskCount === 0 && completed.length === 0 && (
                <EmptyInline
                  icon={ListTodo}
                  title="No tasks yet"
                  body="Add your first task above to start tracking what needs doing today."
                />
              )}
            </div>
          )}
...
function MeetingRow({ meeting, showDate = false, dealName }) {
  ...
  return (
    <div className="flex items-center gap-4 rounded-lg border border-valence-border bg-valence-surface px-4 py-3 hover:bg-valence-elevated hover:border-valence-border-strong transition">
