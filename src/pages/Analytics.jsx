...
import {
  ...
  Handshake, CalendarClock, RotateCcw
} from 'lucide-react'
...
export default function Analytics() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // supabase-js resolves (never throws) on RLS/query failure — check error
      // explicitly so a transient failure surfaces a retry instead of silently
      // rendering the "no deals yet" empty state over a real pipeline.
  ...
  // Loading: don't flash the "no deals yet" empty state (or the chart wall)
  // over an in-flight fetch — show a header + skeleton while data lands.
    ...
    return (
      <div className="space-y-10">
        ...
        <div>
          ...
          <h1 className="mt-2 font-display text-feature font-bold text-valence-text">The firm, in numbers.</h1>
        </div>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-24 rounded-2xl bg-valence-elevated animate-pulse" />)}
        </div>
        <div className="h-64 rounded-2xl bg-valence-elevated animate-pulse" />
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="h-56 rounded-2xl bg-valence-elevated animate-pulse" />
          <div className="h-56 rounded-2xl bg-valence-elevated animate-pulse" />
        </div>
      </div>
    )
  ...
  // Error: a failed/timed-out fetch must not masquerade as an empty pipeline.
  // Offer a retry instead of silently telling a real firm they have no deals.
    ...
    return (
      <div className="space-y-10">
        ...
        <div className="vl-card p-8 text-center max-w-xl mx-auto">
          <AlertTriangle className="mx-auto h-6 w-6 text-valence-danger" />
          <p className="mt-3 text-sm font-semibold text-valence-text">Couldn't load your analytics.</p>
          <p className="mt-2 text-xs text-valence-muted leading-relaxed">{humanError(loadError, 'Something went wrong fetching your pipeline.')}</p>
          <button onClick={load} className="vl-btn-secondary mx-auto mt-5"><RotateCcw className="h-3.5 w-3.5" /> Try again</button>
        </div>
      </div>
    )
  }
  ...
  // deal" prompt instead.
  if (isSupabaseConfigured && !loading && !loadError && deals.length === 0) {
    return (
      <div className="space-y-10">
        <ConfigBanner />
        <div>
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Filters can narrow a populated pipeline down to zero — render a clear
          "no match + clear" state instead of a wall of empty bars and flat lines. */}
      {filteredDeals.length === 0 ? (
        <div className="vl-card p-10 text-center max-w-xl mx-auto">
          <Filter className="mx-auto h-6 w-6 text-valence-muted" />
          <p className="mt-3 text-sm font-semibold text-valence-text">No deals match this scope.</p>
          <p className="mt-2 text-xs text-valence-muted leading-relaxed">
            Nothing in <b className="text-valence-text">{period}</b>
            {sectorFilter !== 'all' && <> · <b className="text-valence-text">{sectorFilter}</b></>}. Widen the period or clear the sector filter.
          </p>
          <button
            onClick={() => { setPeriod('LTM'); setSectorFilter('all') }}
            className="vl-btn-secondary mx-auto mt-5"
          ><RotateCcw className="h-3.5 w-3.5" /> Reset filters</button>
        </div>
      ) : (
      <>
      {/* ── Pipeline health · operational, not money ── */}
