...
import { Download, ArrowDownAZ, Clock, Briefcase, AlertTriangle, RotateCcw } from 'lucide-react'
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        // A failed deals query must surface a retry, not silently render as
        // "No deals match these filters" (a false empty over a real pipeline).
    ...
    <div className="mx-auto max-w-6xl px-4 sm:px-6 py-6 sm:py-10">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-valence-border bg-valence-elevated px-6 py-16 text-center">
          <div className="mb-3 inline-grid h-10 w-10 place-items-center rounded-full bg-valence-danger/10 ring-1 ring-valence-danger/30">
            <Briefcase className="h-4 w-4 text-valence-danger" />
          </div>
          <h3 className="text-base font-semibold text-valence-text">Couldn't load the aging report</h3>
          <p className="mt-1 text-sm text-valence-muted">{humanError(error, 'Something went wrong fetching your deals.')}</p>
          <button onClick={() => setReloadKey(k => k + 1)} className="vl-btn-secondary mx-auto mt-5">
        ...
        <div className="rounded-xl border border-valence-border bg-valence-elevated overflow-x-auto">
