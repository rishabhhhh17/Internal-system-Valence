...
### 1.1 Default org for the multi-tenant migration
...
- **What I need from you:** A yes/no on the name. If you'd rather
  use a different name (e.g. "Valence Growth Partners — London" to
  leave room for a separate Mumbai entity later), tell me now.
- **When:** Before I run the migration. I will not run it without
  this confirmation.

### 1.2 Brand decision: Orbit or ValenceOS?
...
- **When:** Before we wire customer-facing communications. ~1 week.

### 1.3 Launch cohort — single firm or multi-tenant from day 1?

- **The ask:** Decide whether v1 launches as a single-tenant deploy
  for Valence's internal use, or a multi-tenant SaaS open to two or
  more firms simultaneously.
- **Why:** Single-tenant cuts ~2 weeks of engineering (no need for
  the org_id migration, RLS rewrite, invite flow). Multi-tenant is
  the right answer if you want this to be a product, not an
  internal tool.
...
### 2.1 Email provider account + key

- **Why:** Sending invites, password resets, allowance notifications,
  and any other transactional email. Right now we have no way to
  email a customer anything.
- **What I need:**
  - Pick one: **Resend** (easiest), **Postmark** (best deliverability
    for transactional), or **AWS SES** (cheapest at scale, more setup).
  - API key for the chosen provider.
- **Recommendation:** Resend for speed; we can re-evaluate at scale.

### 2.2 Sentry account + DSN
...
- **Why:** Understand which features get used vs ignored. Drives what
  to invest in vs cut.
...
- **Why:** `valenceos.vercel.app` is fine for the pilot but the team
  you sell to will Google it before they click. A real domain
  signals seriousness.
- **What I need:**
...
- **The ask:** Get a lawyer (yours or external) to review the starter
  Terms of Service and Privacy Policy I've drafted and either approve
  or rewrite.
- **Why:** Cannot publicly launch without lawyer-reviewed terms. IB
  firms specifically will ask for a Data Processing Agreement (DPA)
  before signing — counsel needs to draft that too.
- **What I need:** Confirmation that legal review is in motion, and
  a rough ETA. Whoever you use, send them the current `/terms` and
  `/privacy` pages and ask for a redline.
...
- **The ask:** Go into Google AI Studio, revoke the current
  `VITE_GEMINI_API_KEY`, generate a new one, give it to me to set as
  `GEMINI_API_KEY` (server-side env var only — won't leak this time).
...
- **The ask:** 1–3 firms you'd be comfortable putting on this as a
  pilot in the first 30 days.
- **Why:** Real usage from real partners is the only way to find
  rough edges before they go public.
...
- **The ask:** A lawyer who's done SaaS terms before. Doesn't need
  to be expensive — a junior associate at a tech-friendly firm is
  fine for v1.
- **Why:** §3.1.
...
- **The ask:** A specific person who can sign up for Resend, Sentry,
  PostHog, the domain — and who has authority to put them on a
  Valence corporate card.
...
| Valence is the operating entity behind v1 (not a separate Orbit Inc.) | □ |
...
|---|---|---|
| 1.1 default org name | 24 hours | Multi-tenant migration → auth → onboarding redirect → real launch path |
| 1.2 brand decision | 24 hours | All customer-facing copy, OAuth URLs, domain pointing |
| 2.1 email provider key | 24 hours after handoff | Transactional emails, allowance warnings, invites |
| 2.4 domain DNS access | 1 hour | Customers see a real URL |
...
- Wiring the remaining 7 AI features through the server proxy (the
  ones still calling Google directly with the leaked key).
- Building the customer-facing settings screens.
